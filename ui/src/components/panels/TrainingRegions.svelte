...
{#snippet headerExtra()}
  <label class="toggle-label">
    <input type="checkbox" onclick={(e) => e.stopPropagation()} bind:checked={layerVisibility.training} />
  </label>
{/snippet}

<AccordionSection title="Training regions" count={features.length} {headerExtra}>
...
<style>
  ...
  .toggle-label input { accent-color: var(--primary-accent); cursor: pointer; flex-shrink: 0; }
