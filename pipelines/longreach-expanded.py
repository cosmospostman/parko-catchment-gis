"""pipelines/longreach-expanded.py — Longreach full expansion bbox pipeline.
...
40,996 pixels in the expansion scene (longreach-expanded.yaml).

Usage
-----
    python -m pipelines.longreach-expanded
    python -m pipelines.longreach-expanded --no-plots
"""
...
OUT_DIR = PROJECT_ROOT / "outputs" / "longreach-expanded"
FEATURES = ["nir_cv", "rec_p", "re_p10"]

TRAIN_LOC_ID = "longreach"
SCENE_LOC_ID = "longreach-expansion"
...
def run(plots: bool = True) -> None:
    ...
    save_pixel_ranking(scored, out_dir / "longreach_expanded_pixel_ranking.csv", FEATURES)
    ...
    if plots:
        ...
        plot_prob_heatmaps(
            scored, scene_loc, out_dir,
            stem="longreach_expanded",
            wms_width=2048,
