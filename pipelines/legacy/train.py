...
FIXTURE_BANDS: list[str] = ["B03", "B04", "B05", "B07", "B08", "B8A", "B11", SCL_BAND, "AOT"]
...
# Bbox derived from the actual science point extent with a small margin.
# Kept tight so the STAC search only returns tiles that genuinely cover the
# points — a loose bbox pulls in adjacent tiles whose UTM grids don't overlap
# the points, causing every COG read to fail with an out-of-bounds window error.
# All science points fall within the Mitchell River Gulf Plains QLD region
# (~141.56–141.60E, 14.64–14.67S), a known Parkinsonia infestation site.
FIXTURE_BBOX: list[float] = [141.54, -14.68, 141.61, -14.63]
# Three years chosen to represent distinct rainfall regimes for Gulf Plains QLD:
#   2021 — dry year, 2022 — average, 2025 — recent
