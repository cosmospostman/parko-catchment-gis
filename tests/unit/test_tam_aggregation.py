...
def _get_prob(df: pl.DataFrame, pid: str) -> float:
    return float(df.filter(pl.col("point_id") == pid)[0, "prob_tam"])
...
def test_single_year_returns_mean_prob():
    ...
    assert len(df) == 1
    assert _get_prob(df, "px1") == pytest.approx(0.7)
...
def test_end_year_gets_full_weight():
    ...
    assert _get_prob(df, "px1") == pytest.approx(0.9)
...
def test_older_year_discounted():
    ...
    recent_prob = _get_prob(df, "recent")
    old_prob    = _get_prob(df, "old")
...
def test_weighted_average_correctness():
    ...
    df = aggregate_year_probs(pids, years, probs, end_year=end_year, decay=decay)
    assert _get_prob(df, "px1") == pytest.approx(expected, rel=1e-5)
...
def test_future_year_does_not_crash():
    ...
    assert 0.0 <= _get_prob(df, "px1") <= 1.0
...
def test_decay_zero_equals_uniform_mean():
    ...
    assert _get_prob(df, "px1") == pytest.approx(
