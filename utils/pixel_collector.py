...
def collect(
    ...
    for shard_idx, shard_points in enumerate(shards):
        ...
        if _shard_complete(shard_idx):
            ...
            if sorted_sp.exists():
                ...
                sort_parquet_by_pixel(shard_path, sorting_tmp, row_group_size=5_000_000, ram_budget_gb=12.0)
    ...
    for p in written_paths:
        if not is_pixel_sorted(p, n_check=10):
            ...
            sorting_tmp.unlink(missing_ok=True)
            sort_parquet_by_pixel(p, sorting_tmp, row_group_size=5_000_000, ram_budget_gb=12.0)
